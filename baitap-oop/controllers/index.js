...
function renderTableNhanVien (mangNhanVien){
    ...
    for(var i = 0 ; i<mangNhanVien.length; i++){
        ...
        `
        <tr>
            <td>${index.maNhanVien}</td>
            <td>${index.tenNhanVien}</td>
            <td>${index.chucVu}</td>
            <td>${index.salary}</td>
            <td>${index.totalSalary()}</td>
            <td>${index.hours}</td>
            <td>${index.xepLoaiNhanVien()}</td>
            <td>
                <button class = "btn btn-danger" onclick = "xoaNhanVien('${index.maNhanVien}')">Xóa</button>
                <button class = "btn btn-primary" onclick = "suaNhanVien('${index.maNhanVien}')">Chỉnh sửa</button>
            </td>
        </tr>
        `
    }
